...
fun SessionRepository.findUpcomingSession(generation: Int, today: LocalDate): Session? {
    return findTopByGenerationAndSessionTimeIsAfterOrderBySessionTimeAsc(generation, today)
}

interface SessionRepository : JpaRepository<Session, Long> {
    fun findAllByGeneration(generation: Int): List<Session>

    fun findTopByGenerationAndSessionTimeIsAfterOrderBySessionTimeAsc(generation: Int, today: LocalDate): Session?
}
